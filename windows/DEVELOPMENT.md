...
“开机自启动（登录时）”默认关闭，开启后通过当前用户的 Windows Run 注册表项启动，无需管理员权限；再次点击即可移除。自启动使用当前程序及启动配置的完整路径，保留 `--config` 指定的配置，不依赖登录时的工作目录。移动应用或改用新版本后需重新开启。若 Windows 任务管理器禁用了该启动项，还需在系统“启动应用”设置中启用。自动测试只操作隔离的临时注册表键，不修改用户的真实自启动状态。

“编辑当前模型场景…”打开共享查看器的场景编辑器，可叠加外置图片或已有背景，调整位置、缩放和图层顺序，保存后热同步至所有显示器。场景按模型保存在工程的本机配置中，详见 [手动场景层](../docs/scenes.md)。
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/package-windows-release.ps1 -Version 0.1.0
```

脚本从当前提交导出源码、重新构建前端和两个架构的宿主，生成 ZIP、提交标识及 SHA-256 校验文件。发布包不含游戏资源、开发机启动配置或 Node.js；用户解压后通过 `Start-Wallpaper.cmd` 安装依赖、应用 Spine 补丁并生成本机启动配置，无需 .NET SDK。首次启动需联网，具体步骤见包内 `README-WINDOWS.txt`。打包目录已存在时脚本拒绝覆盖。
