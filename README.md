...
- 使用 `arts/ui` 原始界面素材提供可拖动桌面时钟，内置罗德岛、孤星、彩虹六号、火山假日和透明黑白重字主题
...
桌面时钟位于“壁纸外观、尺寸与位置”设置中，可开关显示、切换主题，并为任意主题启用类似游戏主页卡片的 3D 透视。需要调整位置时，先取消“锁定时钟位置”，再从菜单启用壁纸交互，即可直接拖动时钟；松开后坐标会自动保存。锁定时钟后，其区域不会拦截角色点击交互。
